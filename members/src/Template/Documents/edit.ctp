...
<div class="panel panel-success">
	...
	<div class="panel-body">
		<?php echo $this->element('Usermgmt.ajax_validation', ['formId'=>'editDocumentForm', 'submitButtonId'=>'editDocumentSubmitBtn']); ?>
		<?php echo $this->Form->create($documentEntity, ['type'=>'file', 'id'=>'editDocumentForm', 'class'=>'form-horizontal']); ?>
				...
				<div class="attachment-section">
					...
					<div class="all-attachments">
						...
						if(!empty($documentEntity['doc_file'])) {
							$this->Form->unlockField('Documents.existing_document_files');
							$attachments = array($documentEntity['doc_file']);
							$fullpath = WWW_ROOT."library".DS.'documents'.DS.$userId.DS;
							$attachedFilesTmp = array();
							foreach($attachments as $k=>$attachedFile) {
								if(file_exists($fullpath.$attachedFile)) {
									$filemtime = filemtime($fullpath.$attachedFile);
									$attachedFilesTmp[$filemtime] = $fullpath.$attachedFile;
									echo $this->Form->hidden('Documents.existing_document_files.'.$k, array('value'=>$attachedFile, 'class'=>'existing_attachments', 'autocomplete'=>'off'));
								}
							}
							...
							foreach($attachedFilesTmp as $attachedFile) {
								...
								echo "<div class='atchFileDiv'>";
									...
									echo "<span class='atchFileName'><a href='".SITE_URL."library/documents/".$userId."/".$path_parts['basename']."' target='_blank'>".$path_parts['basename']." (".$filesize." KB)</a></span>";
									echo "<span class='atchFileDel'><a filename='".$path_parts['basename']."' href='#' class='delExistingAttachment'>".$this->Html->image('blank.gif', array('class'=>'icon'))."</a></span>";
								echo "</div>";
							}
						}
		...
		<div class="um-button-row">
			<?php echo $this->Form->Submit(__('Update Document'), ['div'=>false, 'class'=>'btn btn-primary', 'id'=>'editDocumentSubmitBtn']); ?>
		</div>
